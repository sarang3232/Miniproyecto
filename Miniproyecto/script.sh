...
cd consulService/app1 && sudo npm install consul express
cd ../..
...
sudo echo "consul agent -ui -server -bootstrap-expect=1 -data-dir=. -node=agent-one -bind=192.168.100.3 -client=0.0.0.0 -enable-script-checks=true -config-dir=/etc/consul.d" >> consulagent
